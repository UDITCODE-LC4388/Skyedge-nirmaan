...
def main():
    ...
    try:
        while True:
            ...
            # Compute win_mv and win_urg for the arbitrated event winner
            win_key = event_type if event_type in mission_values else aliases.get(event_type, event_type)
            win_mv = mission_values.get(win_key, routine_mv)
            win_urg = urgencies.get(win_key, routine_urg)

            # Generate real, detailed AI and sensor justification (no synthetic/mock values)
            sensor_summary = ""
            if telemetry:
                temp = telemetry.get("temperature")
                volt = telemetry.get("voltage")
                if temp is not None and volt is not None:
                    sensor_summary = f" | T={temp:.1f}C V={volt:.1f}V"

            if action == "TRANSMIT_NOW":
                justification = (
                    f"Model={winner} | Event={event_type.upper()} | Conf={confidence:.2f} | "
                    f"Sev={severity:.2f} | Score={priority_score:.1f}>={thresholds.get('transmit_now', 90)} | "
                    f"BW={resource_mgr.bandwidth}{sensor_summary}"
                )
            elif action == "COMPRESS_AND_QUEUE":
                justification = (
                    f"Model={winner} | Event={event_type.upper()} | Conf={confidence:.2f} | "
                    f"Score={priority_score:.1f} queued (BW={resource_mgr.bandwidth})"
                )
            elif action == "QUEUE_OR_STORE":
                justification = (
                    f"Low priority {priority_score:.1f} < {thresholds.get('compress_and_queue', 70)}. Stored locally."
                )
            elif action == "STORE_OR_DISCARD":
                justification = (
                    f"Discarded: Score {priority_score:.1f} below threshold {thresholds.get('queue_or_store', 40)}."
                )
            else:
                justification = f"Action {action} on score {priority_score:.1f}."

            # Log audit decision locally
            esp32.send_audit(event_type, action, justification)
            ...
            if resource_mgr.bandwidth != "CRITICAL":
                while queue.peek() is not None:
                    ...
                    if queued_item:
                        if len(queued_item) == 8:
                            packet_path, q_score, q_event, q_conf, q_sev, q_urg, q_mv, q_act = queued_item
                            print(f"[QUEUE DRAIN] Transmitting queued item: {q_event} (score={q_score:.1f})")
                            drain_justification = (
                                f"Queue Drain: Event={q_event.upper()} | Conf={q_conf:.2f} | "
                                f"Score={q_score:.1f} | BW recovered to {resource_mgr.bandwidth}"
                            )
                            esp32.send(
                                packet_path,
                                q_score,
                                ...
                                mission_value=q_mv,
                                action=q_act,
                                justification=drain_justification,
                            )
                        else:
